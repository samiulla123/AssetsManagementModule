...
<%
	...
	ArrayList depuni=db.SelectUnit();
	System.out.println("REsult  found "+depuni);
	String dep="";
	String unit="";
	String prog_id="";
	String main_catName="";
	for(int m=0;m<depuni.size();m++)
	{
		...
		System.out.println("Unit name "+depU.get(0));
	}
...
<%} %>
 ...
 <%for(int u=0;u<depuni.size();u++)
 { 
 	ArrayList unName=(ArrayList)depuni.get(u);
 	String depName=""+unName.get(0);
 	ArrayList dn=db.SelectDepartment(depName);
 	for(int d=0;d<dn.size();d++)
 	{
 	ArrayList department=(ArrayList)dn.get(d);
 	String departName=""+department.get(1);
 	String departId=""+department.get(0);
 	String departSHName=""+department.get(2);
 	%>
 	<br>Unit : <font size="2px" color="blue"><%=unName.get(0)%></font> &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;  PHYSICAL LOCATION : <font size="2px"><b><%=departName %></b></font>
	...
	{ 
		...
		<th>Year</th>
			...
			for(int j=0;j<sub_cat.size();j++)
			{
				...
				System.out.println("Prog id "+departId);
				ArrayList asset=db.selectReportAssetRegisterUnitWise(s_cat.get(0).toString(),departId);
	...
	<%} %>
	<center><a href="AssetUnitPrint.jsp?unit_name=<%=unName.get(0)%>%>&prog_id=<%=departId%>&dep_name=<%=departSHName %>" target="popup" 
				onclick="window.open('AssetUnitPrint.jsp?unit_name=<%=unName.get(0)%>&prog_id=<%=departId%>&dep_name=<%=departSHName %>','popup','width=600,height=600,location=no,scrollbars=yes,resizable=yes'); return false;"><label style="font-size:20px; font-family: serif;">Preview</label></a></center>
	<hr class="new4">
	<%} %>
	<br/><br/>
	<hr class="new5">
		<center><font style="font-size: 20px; font-family: sans-serif; font-style: oblique; size: 50px; color: red;">New Unit Details</font></center>
	<hr class="new5">
<%}%>
</div>
