...
<hr/>
 <%for(int i=0;i<main_cat.size();i++)
	{ 
		...
		ArrayList ass=db.TRMainCatCheck(email,m_det.get(0).toString());
		...
		{ %>
			...
			<table class="table" id="tbl">
				...
				try{
				 ...
				 System.out.println("Sub category "+sub_cat); 
				for(int j=0;j<sub_cat.size();j++)
				{
					ArrayList s_cat = (ArrayList)sub_cat.get(j);
					ArrayList asset=db.selectReportAssetRegisterTransfer(email,s_cat.get(0).toString());
	     		 ...
	     		 %>
						<tr class="Active">
							...
							<% ArrayList TrDetail=db.TRDepRefNo(AR.get(0).toString());
							System.out.println("For loop "+TrDetail+" "+TrDetail.isEmpty());
							String ref_numb="";
								String Depart="";
							if(!TrDetail.isEmpty())
							{
								for(int d=0;d<TrDetail.size();d++)
								{
									ArrayList dt=(ArrayList)TrDetail.get(d);
									System.out.println("id "+dt.get(1));
									System.out.println("For loop"+dt.get(0));
									ref_numb=""+dt.get(1);
									Depart=""+dt.get(0);
								}
							}
							System.out.println("out only ");
								%>
							<td><%=Depart %></td>
							<td><%=AR.get(8) %></td>
							<td><%=AR.get(29) %></td>
							...
							<td><%=AR.get(11)%></td>
							<%
							System.out.println("out only 12");
							System.out.println("out cat "+AR.get(0).toString());
							ArrayList s_date=db.TRDate(email, AR.get(0).toString()); 
