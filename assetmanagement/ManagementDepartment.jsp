...
function displ(id)
{
	...
	alert(document.data.search.value);
		...
		String data=request.getParameter("Unit");
		...
		<div class="">
			<div class="form-group">
				<div class="col-sm-5">
					...
					<center><h3><b>Unit Name</b>:<%=data %></h3></center>
					<hr>	<tr>
							<th>Unit</th>
							<th>Number Of<br/> Assets</th>
						</tr>
						<%	ArrayList Depart=assetManager.selectDepart(data);
							System.out.println("Department "+Depart);
							for(int a=0;a<Depart.size();a++)
							{
								ArrayList dep=(ArrayList)Depart.get(a);

									int numb=assetManager.sumAssetInDepart(dep.get(0).toString());
									System.out.println("Data fetch is "+dep.get(1));
									System.out.println("Main category "+dep.get(0));
									String dep1=""+dep.get(1);
									%>
									<tr>
									<td>
										<input type="hidden" value="<%=dep1%>" name="search">
									<a href="ManagementMainCategory.jsp?main_cat=<%=dep.get(0)%>&dep_name=<%=dep.get(1)%>"><%=dep1%></a></td>
									<td><%=numb %></td>
									</tr>
							<% }%>
					</table>
